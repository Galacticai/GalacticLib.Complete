...
namespace ScreenFIRE.Lib.Math.Space.Space2D.Drawing {
    ...
    public static class Draw {
        ...
        public static void ClearCanvas(this c.Context context, Gtk.Window window) {
            if (Platform.RunningLinux) {
                context.Save();
                context.Operator = c.Operator.Clear;
        ...
        public static void Arrow(this c.Context context, c.PointD origin, c.PointD head) {
            double length = Geometry.Distance(origin, head),
                   angle = Pi / 4,
                   head_angle = Pi / 6,
